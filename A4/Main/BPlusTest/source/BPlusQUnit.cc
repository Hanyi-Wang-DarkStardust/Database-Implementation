...
////            DEBUG
//            supplierTable.loadFromTextFile("supplierSmall.tbl");
...
//            exit(0);
        ...
        case 7: {
            ...
            FALLTHROUGH_INTENDED;
        case 8: {
            cout << "TEST 8... creating tree for small table, on comment, running point queries with no answer "
                 << flush;
            MyDB_BufferManagerPtr myMgr = make_shared<MyDB_BufferManager>(1024, 128, "tempFile");
            MyDB_BPlusTreeReaderWriter supplierTable("comment", myTable, myMgr);
            supplierTable.loadFromTextFile("supplier.tbl");
            ...
            FALLTHROUGH_INTENDED;
        case 9: {
            cout << "TEST 9... creating tree for small table, on suppkey, running point queries under and over range "
                 << flush;
            MyDB_BufferManagerPtr myMgr = make_shared<MyDB_BufferManager>(1024, 128, "tempFile");
            MyDB_BPlusTreeReaderWriter supplierTable("suppkey", myTable, myMgr);
            supplierTable.loadFromTextFile("supplier.tbl");
